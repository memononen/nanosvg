...
void nsvgRasterize(NSVGrasterizer* r,
				   ...
				   unsigned char* dst, int w, int h, int stride, recursive_image = NULL, const void *obj = NULL);
...
static void nsvg__flattenShapeStroke(NSVGrasterizer* r, NSVGshape* shape, float scalex, float scaley)
{
	...
	float lineWidth = shape->strokeWidth * (scalex + scaley)/2.f;
	...
	for (path = shape->paths; path != NULL; path = path->next) {
		...
		if (shape->strokeDashCount > 0) {
			...
			dashLen = (shape->strokeDashArray[idash] - dashOffset) * (scalex + scaley)/2.f;
			...
			for (j = 1; j < r->npoints2; ) {
				...
				if ((totalDist + dist) > dashLen) {
					...
					dashState = !dashState;
					idash = (idash+1) % shape->strokeDashCount;
					dashLen = shape->strokeDashArray[idash] * (scalex + scaley)/2.f;
					// Restart
					cur.x = x;
					cur.y = y;
